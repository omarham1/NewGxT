...
**Structural Canvas**:
The foundation Visual Dependency Phase of the indicator: Relevant Levels at the same prices across monitored chart timeframes (Daily through 1m), with Session POI / Continuation POI visually emphasized when selected. PDH/PDL and PWH/PWL are hard always-on (no hide toggle). HTF Swing Points and HTF FVGs are toggleable (default on) — swings as labeled solid lines on every timeframe when shown; HTF FVGs as shaded zones on their native 4H and 1H charts only. Active DOL TP1/TP2 badges are toggleable (default on). On-chart labels use a priority stack (Session/Continuation POI > Active DOL > PWH/PWL > PDH/PDL > HTF swings > Session Context); the winning label keeps its type stem (e.g. `PDH · POI`).
_Avoid_: Background levels, static overlay

**Session Context**:
The session reference Visual Dependency Phase sitting above Structural Canvas: 18:00 Daily Open (dashed labeled line), ADR band (faint dashed lines at open ± ADR with consumption label on ADR+ only), PD 50% Midpoint, and PD Equilibrium Range band. Daily Open, ADR, and PD 50% Midpoint are toggleable (default on); PD Equilibrium Range is toggleable (default off). These are not Relevant Levels and are never Session POI.
