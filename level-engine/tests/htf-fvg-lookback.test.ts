...
describe("isWithinHtfFvgLookback", () => {
  ...
  it("returns false when formation time is two or more CME weeks before asOf", () => {
    expect(isWithinHtfFvgLookback(SUN_DEC_29_OPEN, SUN_JAN_12_OPEN)).toBe(false);
  });

  it("drops gaps from the third week back at the Sunday 18:00 ET week roll", () => {
    expect(isWithinHtfFvgLookback(FRI_JAN_3_CLOSE, SUN_JAN_12_OPEN)).toBe(false);
    expect(isWithinHtfFvgLookback(SUN_JAN_5_OPEN, SUN_JAN_12_OPEN)).toBe(true);
  });

  it("keeps a prior-week Friday close gap visible through the following week", () => {
    expect(isWithinHtfFvgLookback(FRI_JAN_3_CLOSE, MON_JAN_6_EVAL)).toBe(true);
  });
