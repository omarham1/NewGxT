...
2. **Decoupled Sync Gate**:
   - When triad assets are decoupled (pulling apart / not expanding together), wait for one asset to interact with a Relevant Level before treating a Strength Switch or Catch-Up Play as high quality. The level interaction resolves the tug-of-war; then existing Target Matching, Lead, Catch-Up, and Continuation rules apply unchanged.

3. **Primary Selection (Lead Asset)**:
...
5. **Secondary Selection ([Catch-Up Play](file:///Users/omarhamouda/Projects/NewGxT/CONTEXT.md#L78) to the High/Low)**:
...
6. **Tertiary Selection ([Continuation Play](file:///Users/omarhamouda/Projects/NewGxT/CONTEXT.md#L82) beyond the High/Low)**:
...
- Early Strength Switch or Catch-Up signals during decoupled opens are filtered by the Decoupled Sync wait until one asset interacts with a Relevant Level.
